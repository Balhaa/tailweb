...
import DaftarImage from './DaftarImage.json';

const Imagegallery = () => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const { images } = DaftarImage;
  ...
  return (
    <div className="image-gallery">
      ...
      <div className="photo-container">
        <img
          src={images[currentIndex]} // Use 'images' instead of 'Image'
          alt={`Image ${currentIndex + 1}`}
          className="gallery-image rounded-full w-82 h-82" // Adjust the size as needed
        />
      </div>
